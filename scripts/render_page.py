#!/usr/bin/env python3
"""
Shared headless renderer for seo-agents.

Every subagent that fetches HTML for analysis (sb-tech-audit,
sb-content-quality, sb-schema-audit, sb-ai-search, sb-sxo) calls this module
instead of ``fetch_page.py`` whenever JS execution might change what an audit
can see. Built on Playwright Chromium with trafilatura for boilerplate-free
content extraction and htmldate for publication-date detection.

Why
===
Fetching raw HTML produces false negatives on SPAs (empty
``<div id="root">``, no schema in source, no content in source). This module
solves that once, as a shared subsystem for every agent that reads pages.
...
is noted below in this docstring.
...
USER_AGENT = (
    ...
    "(KHTML, like Gecko) Chrome/131.0.0.0 Safari/537.36 SEOAgents/1.0"
)
...
def render_page(
    ...
    *,
    ...
    captured and attached to ``result['accessibility_tree']`` for
    agent-friendliness scoring (Google AI optimization guide / web.dev
    agent UX criteria).
    """
    result: dict = {
        "url": url,
...
def _cli() -> None:
    parser = argparse.ArgumentParser(
        description="seo-agents shared headless renderer (Playwright + trafilatura)"
    )
