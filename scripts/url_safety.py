#!/usr/bin/env python3
"""
Canonical URL safety module for seo-agents.
...
seo-agents scripts are intentionally single-threaded; parallelism is
...
The residual DNS-rebinding risk for browser-based fetches is accepted
as a known limitation: Chromium may re-resolve DNS between validation and fetch.
"""
...
# This is a deliberate choice: seo-agents scripts run one URL fetch at a
...
def _pin_dns(hostname: str, pinned_ip: str, port: int) -> Iterator[None]:
    ...
    The fall-through validation closes the redirect-target rebinding gap:
    ``requests.Session.get(allow_redirects=True)`` follows 30x redirects
    to a *different* hostname; without this guard, the redirect target
    would be resolved by the unpatched resolver and could land on a
    private IP.
    ...
    if not _dns_patch_lock.acquire(blocking=False):
        raise URLSafetyError(
            ...
            "seo-agents url_safety is not thread-safe by design."
        )
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Validate a URL against seo-agents's SSRF policy."
    )
